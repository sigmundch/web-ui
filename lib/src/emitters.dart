...
class ElementFieldEmitter extends Emitter<ElementInfo> {
  ...
  void emitCreated(Context context) {
    ...
    var childrenCreatedInCode = info.children.some((c) => c.createdInCode);

    if (info.createdInCode) {
      var exp = _createElementExpression(info.node, !childrenCreatedInCode);
      printer.add("${info.identifier} = $exp;");
    } else if (!info.isRoot) {
    ...
    if (childrenCreatedInCode && !info.hasIterate && !info.hasIfCondition) {
...
class ContentFieldEmitter extends Emitter<TextInfo> {
  ...
  void emitCreated(Context context) {
    context.createdMethod.add('''
        ${info.identifier} = ${info.binding} is autogenerated.SafeHtml
          ? new autogenerated.Element.html(${info.binding}.toString())
          : new autogenerated.Text(${info.binding}.toString());
    ''');
  }
  ...
  void emitDeclarations(Context context) {
    info.events.forEach((name, events) {
      for (var event in events) {
        var listenerName = '_listener${info.identifier}_${name}_';
...
/**
 * Generates watchers that listen on data changes and update an element's
 * attributes.
 */
abstract class AttributeEmitter extends Emitter<ElementInfo> {
  AttributeEmitter(ElementInfo info) : super(info);

  bool shouldEmit(AttributeInfo info);
  void emitAttributeInserted(Context context, String name, AttributeInfo attr);

  /** Emit a field for each disposer function. */
  void emitDeclarations(Context context) {
    info.attributes.forEach((name, attr) {
      if (!shouldEmit(attr)) return;

      attr.stopperNames = [];
      attr.bindings.forEach((b) {
        var stopperName = newName(context, '_stopWatcher${info.identifier}_');
        attr.stopperNames.add(stopperName);
  ...
  void emitInserted(Context context) {
    info.attributes.forEach((name, attr) {
      if (!shouldEmit(attr)) return;
      emitAttributeInserted(context, name, attr);
    });
  }
  ...
  void emitRemoved(Context context) {
    info.attributes.forEach((name, attr) {
      ...
      attr.stopperNames.forEach((stopperName) {
...
class ClassAttributeEmitter extends AttributeEmitter {
  ClassAttributeEmitter(ElementInfo info) : super(info);

  bool shouldEmit(AttributeInfo attr) => attr.isClass;

  void emitAttributeInserted(Context context, String name, AttributeInfo attr) {
    for (int i = 0; i < attr.bindings.length; i++) {
      var stopperName = attr.stopperNames[i];
      var exp = attr.bindings[i];
      context.insertedMethod.add('''
          $stopperName = autogenerated.watchAndInvoke(() => $exp, (e) {
            if (e.oldValue != null && e.oldValue != '') {
              ${info.identifier}.classes.remove(e.oldValue);
            }
            if (e.newValue != null && e.newValue != '') {
              ${info.identifier}.classes.add(e.newValue);
            }
          });
      ''');
    }
...
class DataStyleAttributeEmitter extends AttributeEmitter {
  DataStyleAttributeEmitter(ElementInfo info) : super(info);

  bool shouldEmit(AttributeInfo attr) => attr.isStyle;

  void emitAttributeInserted(Context context, String name, AttributeInfo attr) {
    // TODO(jmesserly): we should throw a different kind of error.
    // Probably we need a data-binding error of some sort.
    var val = attr.boundValue;
    var stopperName = attr.stopperNames[0];
    context.insertedMethod.add('''
        $stopperName = autogenerated.watchAndInvoke(() => $val, (e) {
          if (e.oldValue is Map<String, String>) {
            var __props = e.newValue;
            if (__props is! Map<String, String>) __props = const {};
            for (var property in e.oldValue.keys) {
              if (!__props.containsKey(property)) {
                // Value will not be overwritten with new setting. Remove.
                ${info.identifier}.style.removeProperty(property);
              }
            }
          }
          if (e.newValue is! Map<String, String>) {
            throw new ArgumentError("Expected Map<String, String> value "
                "to data-style binding '$val'.");
          }
          e.newValue.forEach(${info.identifier}.style.setProperty);
        });
    ''');
  }
}

class SimpleAttributeEmitter extends AttributeEmitter {
  SimpleAttributeEmitter(ElementInfo info) : super(info);

  bool shouldEmit(AttributeInfo attr) => attr.isSimple;
  ...
  void emitAttributeInserted(Context context, String name, AttributeInfo attr) {
    ...
    var stopperName = attr.stopperNames[0];
    context.insertedMethod.add('''
        $stopperName = autogenerated.watchAndInvoke(() => $binding, (e) {
          ${info.identifier}.$setter = e.newValue;
        });''');
  }
}

class TextAttributeEmitter extends AttributeEmitter {
  TextAttributeEmitter(ElementInfo info) : super(info);

  bool shouldEmit(AttributeInfo attr) => attr.isText;
  ...
  void emitAttributeInserted(Context context, String name, AttributeInfo attr) {
    ...
    context.insertedMethod.add('''
        $stopperName = autogenerated.watchAndInvoke(() => $binding, (e) {
          ${info.identifier}.$setter = '$content';
        });''');
  }
}

/** Generates watchers that listen on data changes and update text content. */
class ContentDataBindingEmitter extends Emitter<TextInfo> {
  ContentDataBindingEmitter(TextInfo info) : super(info);

  /** Emit a field for each disposer function. */
  void emitDeclarations(Context context) {
    info.stopperName = newName(context, '_stopWatcher${info.identifier}_');
    context.declarations.add(
        'autogenerated.WatcherDisposer ${info.stopperName};');
  }
  ...
  void emitInserted(Context context) {
    ...
    context.insertedMethod.add('''
        ${info.stopperName} = autogenerated.watchAndInvoke(() => $val, (e) {
          var isSafeHtml = e.newValue is autogenerated.SafeHtml;
          var stringValue = e.newValue.toString();
          if (!isSafeHtml && $id is autogenerated.Text) {
            $id.text = stringValue;
          } else {
            var oldNode = $id;
            $id = isSafeHtml ? new autogenerated.Element.html(stringValue)
                : new autogenerated.Text(stringValue);
            oldNode.replaceWith($id);
          }
        });
    ''');
  }
  ...
  /** Call the dispose method on all watchers. */
  void emitRemoved(Context context) {
    context.removedMethod.add('${info.stopperName}();');
  }
}
...
class ConditionalEmitter extends Emitter<TemplateInfo> {
  ...
  void emitDeclarations(Context context) {
    ...
    printer.add('autogenerated.WatcherDisposer _stopWatcher$id;');
  ...
  void emitInserted(Context context) {
    ...
    printer.add('''
        _stopWatcher$id = autogenerated.watchAndInvoke(() => $cond, (e) {
    ...
    if (info.isTemplateElement) {
      printer.add('var __insert_$id = $id.nextNode;');
      for (var child in info.children) {
        var exp = _createChildExpression(child);
        if (child == info.children.last) {
          printer.add('_endPosition$id = $exp;');
          exp = '_endPosition$id';
        }
        printer.add('$id.parent.insertBefore($exp, __insert_$id);');
      }
    } else {
  ...
  void _removeChildNodes(CodePrinter printer) {
    ...
    if (info.isTemplateElement) {
      printer.add('''
          while ($id != _endPosition$id) {
            var prev = _endPosition$id.previousNode;
            _endPosition$id.remove();
            _endPosition$id = prev;
          }
      ''');
    } else {
...
class ListEmitter extends Emitter<TemplateInfo> {
  ...
  Context contextForChildren(Context c) {
    return new Context(_childrenDeclarations, _childrenCreated,
        _childrenInserted, _childrenRemoved);
  }
}
...
class RecursiveEmitter extends InfoVisitor {
  ...
  void visitElementInfo(ElementInfo info) {
    ...
    var emitters = [fieldEmitter,
        ...
        new SimpleAttributeEmitter(info),
        new TextAttributeEmitter(info),
        new ClassAttributeEmitter(info),
        new DataStyleAttributeEmitter(info),
  ...
  String run(ComponentInfo info, PathInfo pathInfo) {
    ...
    if (codeInfo == null) {
      ...
      var imports = hasExtends ? [] : [new DartDirectiveInfo('import',
          'package:web_components/web_components.dart', 'autogenerated')];
...
_createChildExpression(NodeInfo info) {
  if (info.identifier != null) return info.identifier;
  var node = info.node;
  return (node is Text)
    ? "new autogenerated.Text('${escapeDartString(node.value)}')"
    : _createElementExpression(node, true);
}
 ...
 */
String _createElementExpression(Element node, bool includeChildren) {
  ...
  var isEmpty = node.attributes.length == 0 &&
      (node.nodes.length == 0 || !includeChildren);
  ...
  if (isEmpty && elemName != null && isHtml) {
    ...
    if (!includeChildren) node = node.clone();
