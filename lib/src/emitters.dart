 ...
 */
class RecursiveHTMLEmitter extends InfoVisitor {
  final String _startTagName;
  final CodePrinter initialPage = new CodePrinter();
  var parent;
  Node stopEmitting;

  RecursiveHTMLEmitter(this._startTagName);
  ...
  void _emitStartTag(Node node, var elementInfo) {
    if (stopEmitting != null) {
      if (node is Element) {
        initialPage.add("<${node.tagName}${_attributesToString(node)}>");
      } else {
        if (node is Text && elementInfo.contentBinding == null) {
          var textValue = htmlSerializeEscape(node.value);
          initialPage.add("$textValue");
        }
      }
    }
  }
...
/**
 * Only emit minimum HTML all templates will be emmitted in the generated
 * insertion code.
 */
class HtmlEmitter extends RecursiveHTMLEmitter {
  HtmlEmitter(FileInfo info, [String startTagName = null]) :
      super(startTagName);

  String run(ElementInfo root) {
    visit(root);

    var result = initialPage.toString();
...
class WebComponentEmitter extends RecursiveEmitter {
  ...
  String run(ComponentInfo info, PathInfo pathInfo) {
    ...
    var html = new HtmlEmitter(_fileInfo).run(elemInfo);
...
class MainPageEmitter extends RecursiveEmitter {
  ...
  String run(Document document, PathInfo pathInfo) {
    ...
    var htmlEmitter = new HtmlEmitter(_fileInfo, 'body');
    var initialPage = htmlEmitter.run(elemInfo);
