...
class ElementFieldEmitter extends Emitter<ElementInfo> {
  ...
  void emitCreated(Context context) {
    ...
    if (info.createdInCode) {
      var exp = _createNodeExpression(!childrenCreatedInCode);
    ...
    printer.add(childrenCreated);

    if (childrenCreatedInCode) {
      // Co-iterate nodes and children, emitting every node that doesn't
      // have a corresponding NodeInfo.
      var iter = info.children.iterator();
      var childInfo = iter.hasNext ? iter.next() : null;
      for (var node in info.node.nodes) {
        var childExpression;
        if (childInfo != null && node == childInfo.node) {
          // Node already emitted, just add a reference to it:
          childExpression = childInfo.identifier;
          childInfo = iter.hasNext ? iter.next() : null;
        } else if (node is Text) {
          // Text node with no data bindings
          childExpression =
              "new autogenerated.Text('${escapeDartString(node.value)}')";
        } else {
          // HTML node with no data bindings
          childExpression = "new autogenerated."
              "Element.html('${escapeDartString(node.outerHTML)}')";
        }
        printer.add("${info.identifier}.nodes.add($childExpression);");
      }
    }
  }

  /**
   * An (runtime) expression to create the node represented by [info]. It
   * always includes the node's attributes, but only includes children nodes if
   * [includeChildren] is true.
   */
  String _createNodeExpression(bool includeChildren) {
    var node = info.node;
  ...
  Context contextForChildren(Context c) => new Context(
      c.declarations, childrenCreated, c.insertedMethod, c.removedMethod);
}

/** Generates a field for any data-bound text node. */
class TextFieldEmitter extends Emitter<TextInfo> {
  TextFieldEmitter(TextInfo info) : super(info);

  void emitDeclarations(Context context) {
    context.declarations.add('autogenerated.Text ${info.identifier};');
  }

  void emitCreated(Context context) {
    context.createdMethod.add('${info.identifier} = '
 ...
 * attributes.
 */
class AttributeDataBindingEmitter extends Emitter<ElementInfo> {
  AttributeDataBindingEmitter(ElementInfo info) : super(info);
      ...
      if (attrInfo.isClass) {
        ...
        context.insertedMethod.add('''
            $stopperName = autogenerated.watchAndInvoke(() => $val, (e) {
              ...
              e.newValue.forEach($elemField.style.setProperty);
            });
        ''');
      } else {
        ...
        var setter = null;
        var typeName = typeForHtmlTag(info.node.tagName);
        while (typeName != null) {
...
class TextDataBindingEmitter extends Emitter<TextInfo> {
  TextDataBindingEmitter(TextInfo info) : super(info);
  ...
  void emitInserted(Context context) {
    ...
    context.insertedMethod.add('''
        ${info.stopperName} = autogenerated.watchAndInvoke(() => $val, (e) {
          ${info.identifier}.text = $val.toString();
        });
    ''');
  }
...
class ConditionalEmitter extends Emitter<TemplateInfo> {
  ...
  void emitDeclarations(Context context) {
    var id = info.identifier;
    context.declarations.add('''
        // Fields for template conditional '${info.ifCondition}'
  ...
  void emitInserted(Context context) {
    ...
    var id = info.identifier;
    var firstChildId = info.children[0].identifier;

    context.insertedMethod.add('''
        _stopWatcher_if$id = autogenerated.watchAndInvoke(() => $cond, (e) {
              bool showNow = e.newValue;
              if ($firstChildId != null && !showNow) {
                // Remove children''');

    for (var child in info.children) {
      context.insertedMethod.add('${child.identifier}.remove();');
    }

    context.insertedMethod
        .add('// Remove any listeners/watchers on children')
        .add(childrenRemoved)
        .add('} else if ($firstChildId == null && showNow) {')
        .add('// Initialize children')
        .add(childrenCreated)
        .add('var __insert$id = $id.nextNode;');

    for (var child in info.children) {
      context.insertedMethod.add(
          '$id.parent.insertBefore(${child.identifier}, __insert$id);');
    }
  ...
  void emitRemoved(Context context) {
    ...
    for (var child in info.children) {
      context.removedMethod.add('${child.identifier}.remove();');
    }

    context.removedMethod
        .add('// Remove any listeners/watchers on children')
        .add(childrenRemoved)
        .add('}');
  }
...
class ListEmitter extends Emitter<TemplateInfo> {
  ...
  void emitInserted(Context context) {
    ...
    if (info.isTemplateElement) {
      context.insertedMethod.add(
          'var __insert$id = ${info.identifier}.nextNode;');
    }
    ...
    for (var child in info.children) {
      if (info.isTemplateElement) {
        ...
        context.insertedMethod.add('$id.nodes.add(${child.identifier});');
      }
    }
...
class RecursiveEmitter extends InfoVisitor {
  ...
  void visitElementInfo(ElementInfo info) {
    ...
    var emitters = [fieldEmitter,
        ...
        new AttributeDataBindingEmitter(info),
  ...
  void visitTextInfo(TextInfo info) {
    var emitters =
        [new TextFieldEmitter(info), new TextDataBindingEmitter(info)];
    for (var e in emitters) {
      e.emitDeclarations(_context);
      e.emitCreated(_context);
      e.emitInserted(_context);
